...
/**********************************************************************************************/
    ...
    /**
     */
    function getLeagueStats(batterStatNames, pitcherStatNames) {
	  ...
	  setUpTable(tableId, PITCHER);
        ...
        for (var i = 0; i < nodes.snapshotLength; i++) {
		/* team name and url */
		var teamURL  = 'http://baseball.fantasysports.yahoo.com' + nodes.snapshotItem(i).childNodes[0].getAttribute("href");
		...
		statBody.appendChild(totalRow);
		updateStatsRow(new TotalBatter(teamName, i), batterStatNames);

		tableId = STAT_BODY_ID + PITCHER;
		statBody = document.getElementById(tableId);
		totalRow = document.createElement("tr");
		statBody.appendChild(totalRow);
		updateStatsRow(new TotalPitcher(teamName, i), pitcherStatNames);

		GM_xmlhttpRequest({
                method:"GET",
                url:teamURL,
		    onload:function(details) {
			  var statsDocument = new String(details.responseText);

			  getTeamStats(statsDocument, batterStatNames, BATTER, new TotalBatter(teamName, i));
			  getTeamStats(statsDocument, pitcherStatNames, PITCHER, new TotalPitcher(teamName, i));
		    }
		});
	  }
    }
    ...
    */
    function getStats() {
	  ...
	  if (SCRIPT_MODE == 'team') {
		...
		getLeagueStats(batterStatNames, pitcherStatNames);
	  }
...
//Bug Log
//2007-05-06: League wide stats functional if only the xpath parsing of fetching documents works properly --- (FIXME) (RMR)
